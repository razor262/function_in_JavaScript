...
function t5() {
  if (s52.value == "/") {
    return (result = +s51.value / +s53.value);
  }
  if (s52.value == "*") {
    return (result = +s51.value * +s53.value);
  }
...
function t14() {
  document.querySelector(".out-14").style.background = `rgb(${t13(
    0,
    255
  )},${t13(0, 255)},${t13(0, 255)})`;
}
...
document.querySelector(".s-171").addEventListener("change", function (e) {
  document.querySelector(".s-172").value = t17(e);
});
...
document.querySelector(".i-18").addEventListener("input", function (e) {
  document.querySelector(".out-18").textContent = t18(e);
});
...
function t20(s) {
  let current20 = s.split("").reverse().join("");
  console.log(current20);
  return current20;
}
...
function t21(s) {
  //console.log(s)
  // получить значение четное или нечетное
  // если не сходится перезапустить
...
function t26() {
    ...
    let current26 = i26.value
    if (current26 != "") {
       // console.log(current26.toLowerCase())
        out26.innerHTML = current26.toLowerCase()
    }
    //console.log(i26.value)
}
let i26 = document.querySelector('.i-26')
...
function t27(v) {
    ...
    console.log(array27)
    for (let i = 0; i < array27.length; i++) {
        const element = array27[i];
        if (array27[i] != false) {
            return array27[i]
        }
    }
